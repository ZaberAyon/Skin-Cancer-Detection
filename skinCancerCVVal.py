...
def TDS_calc(image):
    ...
    res2=res.reshape((crop_main.shape))
    for clr in center:
    ...
    return tds, result_c, A_minor, A_major, B, C, temp
...
for filename in os.listdir(folder):
        ...
        tds, result, A_minor, A_major, B, C, D = TDS_calc(img)
        image_name.append(filename)
        tds_list.append(tds)
        results.append(result)
        a_minor.append(A_minor)
        a_major.append(A_major)
        diam_pixel.append(D)
        border_irr.append(B)
        color_ind.append(C)
resultDf = pd.DataFrame(list(zip(image_name,tds_list,results,a_minor,a_major,border_irr,color_ind,diam_pixel)),columns=['filename','tds','result','a_minor','a_major','borderirr','colorind','diameter'])
resultDf.to_csv('CVoutput.csv')
cv2.waitKey(0)
